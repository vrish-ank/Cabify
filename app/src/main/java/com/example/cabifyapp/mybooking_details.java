...
public class mybooking_details extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mybooking_details);
        firebaseFirestore = FirebaseFirestore.getInstance();
        mFirestoreList = findViewById(R.id.firestore_list);
        mFirebaseAuth = FirebaseAuth.getInstance();

        String myemail = mFirebaseAuth.getCurrentUser().getEmail();
        Query query = firebaseFirestore.collection("Booker").whereEqualTo("booker_email", myemail);
        ...
        adapter2 = new FirestoreRecyclerAdapter<BookerDisplay, mybooking_details.RequestsViewHolder>(options) {
            @NonNull
            @Override
            public mybooking_details.RequestsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_single3,parent,false);
                return new mybooking_details.RequestsViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull mybooking_details.RequestsViewHolder holder, int position, @NonNull BookerDisplay model) {
